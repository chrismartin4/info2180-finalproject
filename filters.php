...
try {
    ...
    $filter = filter_input(INPUT_GET, 'query', FILTER_SANITIZE_STRING);
    if ($filter == "all") {
    ...
    if ($filter == "open") {
        $titles = "SELECT * FROM issues WHERE status= 'OPEN'";
        $stmt = $conn->query($titles);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    if ($filter == "mine") {
        $titles = "SELECT * FROM issues WHERE assigned_to = '$user_id'";
        $stmt = $conn->query($titles);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
